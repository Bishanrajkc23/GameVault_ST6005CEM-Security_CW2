...
const isPasswordCommon = (password) => {
  const commonPasswords = ["123456789", "password", "abc123", "qwerty", "admin"];
  ...
  if (user) {
    ...
    if (isPasswordValid) {
      ...
      if (user.loginAttempts >= 3) {
        ...
        await user.save();
        return res.status(400).json({ error: "Too many unsuccessful login attempts. Account locked for 1 minutes." });
      } else {
        await user.save();
